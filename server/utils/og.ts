...
const linesToTspans = (lines: string[], x: number, startY: number, lineHeight: number) =>
  lines
    .map((line, index) => {
...
const baseSvg = (content: string) => `<?xml version="1.0" encoding="UTF-8"?>
...
export const renderAppOgSvg = (siteUrl: string) => {
  ...
  <text x="106" y="528" fill="#FDE68A" font-size="32" font-family="Inter, Segoe UI, Arial, sans-serif">${escapeXml(siteUrl)}</text>
  `)
}

export const renderQuoteOgSvg = (saying: Saying, siteUrl: string) => {
  ...
  const footer = `${siteUrl}/${saying.id}`
